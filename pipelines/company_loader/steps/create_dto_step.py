    ...
    def __call__(self, context: Context, next_step: NextStep) -> None:
        if context.raw_company.legal_address:
            en_address = translate_large_text(
                ...
                proxy_obj=get_random_proxy_obj(context.proxies),
            )

        if context.raw_company.legal_name:
            en_legal_name = translate_large_text(
                context.raw_company.legal_name,
                proxy_obj=get_random_proxy_obj(context.proxies),
            )

        if context.raw_company.name:
            en_name = context.raw_company.en_name or translate_large_text(
                context.raw_company.name,
                proxy_obj=get_random_proxy_obj(context.proxies),
            )
