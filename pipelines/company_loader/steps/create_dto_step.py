...
class CreateCompanyDTOStep:
    ...
    def __call__(self, context: Context, next_step: NextStep) -> None:
        if context.raw_company.legal_address:
            en_address = translate(context.raw_company.legal_address, proxies=proxy_settings)

        if context.raw_company.legal_name:
            en_legal_name=translate(context.raw_company.legal_name, proxies=proxy_settings)

        if context.raw_company.name:
            en_name=context.raw_company.en_name or translate(context.raw_company.name, proxies=proxy_settings)
        ...
        dto = CompanyDTO(
            ...
            en_legal_name=en_legal_name,
            legal_address=context.raw_company.legal_address,
            en_legal_address=en_address,
