...
class CountryLegalRequirementPostgresRepository:
    def __init__(self, session: Session):
        self.session = session

    def get_active_country_requirements(
        self, country_code: str, target_date: datetime = None
    ):
        if not target_date:
            target_date = datetime.utcnow()

        return (
            self.session.query(CountryLegalRequirement)
            .filter(
    ...
    def get_country_requirements(self, country_code: str, version: str = None):
        query = self.session.query(CountryLegalRequirement).filter_by(
    ...
    def validate_company_legal_fields(
        self, company_country_code: str, legal_fields: dict
    ):
        requirements = self.get_active_country_requirements(company_country_code)
        if not requirements:
            raise ValueError(
                f"No active legal requirements found for country code: {company_country_code}"
            )
        ...
        for field_name, field_info in legal_fields.items():
            if field_name not in required_fields:
                raise ValueError(
                    f"Field '{field_name}' is not defined for this country."
                )

            required_field = required_fields[field_name]
            field_value = (
                field_info.get("value") if isinstance(field_info, dict) else field_info
            )
    ...
    def _validate_rule(self, rule: ValidationRule, value):
        if rule.validation_type == ValidationType.LENGTH:
            ...
            if len(str(value)) != length:
                raise ValueError(
                    f"Value '{value}' does not meet the length requirement of {length}."
                )
        elif rule.validation_type == ValidationType.REGEX:
            pattern = rule.params.get("pattern")
            if not re.match(pattern, str(value)):
                raise ValueError(
                    f"Value '{value}' does not match the regex pattern: {pattern}."
                )
...
class CompanyRepository:
    ...
    def create_company(
        self, country_code: str, legal_fields: dict = None, translations: dict = None
    ):
        ...
        if translations:
            for field_name, translations_dict in translations.items():
                for lang_code, translated_value in translations_dict.items():
                    self.add_translation_to_company(
                        company.id, field_name, lang_code, translated_value
                    )
    ...
    def add_legal_field_to_company(
        self,
        company_id: UUID,
        field_name: str,
        field_value: any,
        ...
        company = self.get_company_by_id(company_id)
        if company:
            legal_field = company.add_legal_field(
                field_name, field_value, field_type, required
            )
            self.session.commit()
            return legal_field
        return None

    def add_translation_to_company(
        self, company_id: UUID, field_name: str, language_code: str, value: str
    ):
