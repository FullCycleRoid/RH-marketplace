...
from src import LegalStatus, SystemStatus, Company, CompanyFieldType, CompanyField, ValidationType
from src.company.infrastructure.repositories.postgres import CompanyPostgresRepository, FieldTypePostgresRepository
...
class CompanyService:
    ...
    def create_company(
        ...
        country_fields = self.field_type_repository.get_active_country_requirements(country_code)
    ...
    def _validate_and_add_fields(
        ...
        for field_name, field_data in fields.items():
            field_def = next(
                (rf for rf in required_fields if rf.en_name == field_name),
                None
            )
        ...
        for rule in field_def.validation_rules:
            if rule.validation_type == ValidationType.LENGTH:
                ...
                if len(value.get('ru', '')) > max_len:
                    raise ValueError(f"Field {field_def.field_type.ru_name} exceeds maximum length of {max_len}")
                ...
                if not pattern.match(value.get('ru', '')):
                    raise ValueError(f"Field {field_def.field_type.ru_name} doesn't match required pattern")
