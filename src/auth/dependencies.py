...
from src.core.security.jwt import oauth2_scheme, parse_jwt_data_from_oauth2, parse_jwt_data_from_token, refresh_token_scheme
...
from src.auth.service import AuthService, RedisAuthService
from src.auth.units_of_work import SQLAlchemyUsersUnitOfWork
from src.auth.views import AuthViews
...
async def register_user(user_data: RegisterUserScheme) -> User:
    ...
    if not user:
        ...
        user = await auth_service.update_user_profile(
            user=User(
                **(user.to_dict() | user_data.model_dump(exclude={'password'}))
            )
        )
...
async def authenticate_user(jwt_data: JWTData = Depends(parse_jwt_data_from_oauth2)) -> User:
    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
...
async def get_my_account(jwt_data: JWTData = Depends(parse_jwt_data_from_oauth2)) -> User:
...
async def get_user_by_token(jwt_data: JWTData = Depends(parse_jwt_data_from_token)) -> User:
    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
    user: User = await auth_service.get_user_by_id(jwt_data.user_id)
    if not user:
        raise UserNotFound
...
async def create_refresh_token(user: User) -> RefreshToken:
    ...
    if await auth_service.check_old_valid_refresh_token_existence(user=user):
        old_valid_refresh_token: RefreshToken = await auth_service.get_old_valid_refresh_token(user=user)
    ...
    logging.info(f'Created refresh token {new_refresh_token.refresh_token}, which will expire at {new_refresh_token.expires_at}')

    return new_refresh_token


async def valid_refresh_token(refresh_token_value: Optional[str] = Depends(refresh_token_scheme)) -> RefreshToken:
    if not refresh_token_value:
        raise RefreshTokenNotValid

    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
    refresh_token: RefreshToken = await auth_service.get_refresh_token_by_value(value=refresh_token_value)
    if not refresh_token:
        logging.info(
            f'Provided refresh-token {refresh_token_value} was not found'
        )
        raise RefreshTokenNotValid
...
async def get_user_by_refresh_token(refresh_token: RefreshToken = Depends(valid_refresh_token)) -> User:
    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
...
async def expire_refresh_token(refresh_token_value: Optional[str] = Depends(refresh_token_scheme)) -> Optional[RefreshToken]:
    if not refresh_token_value:
        return
...
async def validate_access_token(token: Optional[str] = Depends(oauth2_scheme)) -> bool:
    ...
    if jwt_data.expires.astimezone(tz=timezone.utc) < (datetime.now(tz=timezone.utc) + timedelta(seconds=30)):
        return False
...
async def update_user_profile(
        update_user_data: UpdateUserProfileScheme,
        user: User = Depends(authenticate_user)
) -> User:

    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
    updated_user: User = User(**(user.to_dict() | update_user_data.model_dump(exclude_unset=True)))
...
async def update_user_phone(
        update_phone_data: UpdateUserPhoneScheme,
        user: User = Depends(authenticate_user)
) -> User:
    ...
    updated_user: User = User(**(user.to_dict() | update_phone_data.model_dump(exclude_unset=True)))
...
async def change_old_password(
        password_data: ChangeOldPasswordScheme,
        user: User = Depends(authenticate_user)
) -> User:

    auth_service: AuthService = AuthService(uow=SQLAlchemyUsersUnitOfWork())
    if not check_password(password=password_data.old_password, password_in_db=user.password):
