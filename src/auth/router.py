...
from src.auth.dependencies import change_old_password as change_old_password_dependency
from src.auth.dependencies import update_user_phone as update_user_phone_dependency
from src.auth.dependencies import delete_my_account as delete_my_account_dependency
from src.auth.dependencies import update_user_profile as update_user_profile_dependency
from src.auth.dependencies import validate_access_token as validate_access_token_dependency
from src.auth.models import RefreshToken, User
from src.auth.security import create_access_token, create_forget_password_token, create_verify_email_token
from src.auth.utils import get_cookie_settings
...
async def register(user: User = Depends(register_user)):
    send_verify_email_message.delay(
        token=create_verify_email_token(user=user),
        email_to=user.email
    )
...
@router.get("/verify-email/{token}", status_code=status.HTTP_200_OK, response_model=None)
...
async def refresh_access_token(response: Response, user: User = Depends(get_user_by_refresh_token)):
    response.set_cookie(
        **get_cookie_settings(
            key=auth_config.ACCESS_TOKEN_KEY,
            value=create_access_token(user=user),
...
@router.get("/forget-password/{email}", status_code=status.HTTP_200_OK, response_model=None)
async def forget_password(user: User = Depends(valid_user_email)) -> None:
    send_forget_password_email.delay(email_to=user.email, token=create_forget_password_token(user=user), full_name=user.full_name)


@router.patch("/new-password/{token}", status_code=status.HTTP_200_OK, response_model=None)
async def create_new_password(user: User = Depends(change_forgotten_password)):
    return user.to_dict()


@router.get("/validate-access-token", status_code=status.HTTP_200_OK, response_model=bool)
async def validate_access_token(valid_access_token: bool = Depends(validate_access_token_dependency)):
...
@router.patch("/change-old-password", status_code=status.HTTP_200_OK, response_model=None)
async def change_old_password(user: User = Depends(change_old_password_dependency)):
    return user.to_dict()


@router.delete("/delete-my-account", status_code=status.HTTP_200_OK, response_model=None)
