...
def sync_fetch_one(
    query: Select | Insert | Update, sync_engine: Engine = psycopg_sync_engine
) -> Dict[str, Any] | None:
...
async def fetch_one(
    query: Select | Insert | Update, engine: AsyncEngine = psycopg_engine
) -> Dict[str, Any] | None:
...
async def execute(
    query: Select | Insert | Update | Delete, engine: AsyncEngine = psycopg_engine
) -> Any | None:
    async with engine.begin() as conn:
        try:
...
async def session_fetch_one(
    ...
    session_factory: async_sessionmaker = PsycopgAsyncSessionFactory,
) -> Dict[str, Any] | None:
...
async def execute_bulk(
    ...
    engine: AsyncEngine = psycopg_engine,
) -> None:
    async with engine.begin() as conn:
