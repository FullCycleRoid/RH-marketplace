...
def create_dialogue(dialogue: schemas.DialogueCreate, db: Session = Depends(get_db)):
    ...
    company_a = (
        ...
        .filter(models.Company.id == dialogue.company_a_id)
        .first()
    )
    company_b = (
        db.query(models.Company)
        .filter(models.Company.id == dialogue.company_b_id)
        .first()
    )
    ...
    db_dialogue = models.Dialogue(
        company_a_id=dialogue.company_a_id, company_b_id=dialogue.company_b_id
    )
    db.add(db_dialogue)
    db.commit()
...
@router.get(
    "/dialogues/company_loader/{company_id}",
    response_model=List[schemas.DialogueResponse],
)
def get_dialogues_by_company(company_id: int, db: Session = Depends(get_db)):
    dialogues = (
        db.query(models.Dialogue)
        .filter(
            (models.Dialogue.company_a_id == company_id)
            | (models.Dialogue.company_b_id == company_id)
        )
        .all()
    )
    if not dialogues:
...
@router.get(
    "/dialogues/between/{company_a_id}/{company_b_id}",
    response_model=List[schemas.DialogueResponse],
)
def get_dialogues_between_companies(
    company_a_id: int, company_b_id: int, db: Session = Depends(get_db)
):
    dialogues = (
        db.query(models.Dialogue)
        .filter(
            (models.Dialogue.company_a_id == company_a_id)
            & (models.Dialogue.company_b_id == company_b_id)
            | (models.Dialogue.company_a_id == company_b_id)
            & (models.Dialogue.company_b_id == company_a_id)
        )
        .all()
    )
    if not dialogues:
...
def send_message(message: schemas.MessageCreate, db: Session = Depends(get_db)):
    ...
    dialogue = (
        db.query(models.Dialogue)
        .filter(models.Dialogue.id == message.dialogue_id)
        .first()
    )
    manager = (
        db.query(models.Manager).filter(models.Manager.id == message.manager_id).first()
    )
...
def get_messages_by_dialogue(dialogue_id: int, db: Session = Depends(get_db)):
    messages = (
        db.query(models.Message).filter(models.Message.dialogue_id == dialogue_id).all()
    )
    if not messages:
